...
public class PaymentServlet extends HttpServlet{
    ...
    public PaymentServlet(PaymentService paymentService, RequestValidator requestValidator) {
    ...
    void process(HttpServletResponse resp, String amount, String status, String payload, String timestamp, String md5) throws IOException {
        try {
            requestValidator.validateRequest(amount, status, payload, timestamp, md5);
        } catch (ValidationException2 e) {
            resp.sendError(HttpServletResponse.SC_FORBIDDEN, e.getMessage());
            return;
        }

        //Business logic stars here
        boolean isSBS = false;
        String orderId = null;
